    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#app-form').submit(function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: `{{ route(h_prefix('front_save_app', 1)) }}`,
                    data: formData,
                    cache: false,
                    contentType: false,
                    processData: false,
            ...
            $('#meta-form').submit(function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: `{{ route(h_prefix('front_save_meta', 1)) }}`,
                    data: formData,
                    cache: false,
                    contentType: false,
                    processData: false,
            ...
            $('#meta_list_form').submit(function(e) {
                ...
                const route = ($('#meta_list_id').val() == '') ?
                    "{{ route(h_prefix('front_meta_insert', 1)) }}" :
                    "{{ route(h_prefix('front_meta_update', 1)) }}";
                $.ajax({
                    type: "POST",
                    url: route,
                    data: formData,
        ...
        function meta_list_delete(id) {
            swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: `{{ route(h_prefix('front_meta_delete', 1)) }}`,
                        type: 'DELETE',
                        data: {
                            id
                        },
        ...
        function meta_list_init() {
            $.get(`{{ route(h_prefix('front_meta_list', 1)) }}`, (data) => {
                meta_list_render(data);
                meta_list_is_edit = true;
            });
        }
